"""
test_run_v2.py — Artha Terminal Chat Client  (Multi-Agent Architecture)
========================================================================
Run from the project root:
    python tests/scripts/test_run_v2.py

Identical UX to test_run.py but routes every message through the
Guide → Analyst / Aggregator specialist pipeline defined in multi_agent.py.
...
--------
  ...
  /agents   Show the active multi-agent pipeline
  /help     Show this help
  /quit     Exit
"""
...
# ── Theme: Neon Dusk (same palette, new accent for multi-agent) ───────────────
...
def _banner(username: str, session_id: str):
    log_rel = os.path.join("tests", "logs", f"session_multi_{username}.md")
    _blank()
    _thick()
    print(f"  {_c(BG_ARTHA, '  ⬡  ARTHA  ')}  {_c(WHITE, 'Multi-Agent Financial Analyst')}  "
          f"{_c(MUTED, '· Guide · Analyst · Aggregator')}")
    _thin()
    print(f"  {_c(CORAL, 'User      ')}  {_c(SOFT_PINK, '·')}  {_c(WHITE, username)}")
    print(f"  {_c(CORAL, 'Session   ')}  {_c(SOFT_PINK, '·')}  {_c(MUTED, session_id)}")
    print(f"  {_c(CORAL, 'Pipeline  ')}  {_c(SOFT_PINK, '·')}  "
          f"{_c(BG_AGENT, ' Guide (Groq) ')}  "
          f"{_c(MUTED, '→')}  {_c(SOFT_TEAL, 'Analyst (Gemini)')}  "
          f"{_c(MUTED, '+')}  {_c(SOFT_TEAL, 'Aggregator (Gemini)')}")
...
def _print_agent_pipeline():
    _blank()
    lines = [
        _c(BG_AGENT,  " Guide Agent          ") + "  " + _c(MUTED, "Groq · llama-3.3-70b-versatile"),
        _c(MUTED,     "  ├─ Analyst Agent    ") + "  " + _c(SOFT_TEAL, "Gemini 2.5 Flash Lite · Prices, Fundamentals, Financials"),
        _c(MUTED,     "  └─ Aggregator Agent ") + "  " + _c(SOFT_TEAL, "Gemini 2.5 Flash Lite · News, Web, Docs, Forecasts"),
    ]
    _box(lines, title="Multi-Agent Pipeline", color=SOFT_TEAL)
    _blank()
...
class SessionLogger:
    def __init__(self, username: str):
        ...
        fname     = f"session_multi_{username}.md"
        self.path = os.path.join(_LOGS_DIR, fname)
        self._write(
            f"# Artha Multi-Agent Session — {username}\n\n"
            f"**Started:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
            f"**Pipeline:** Guide (Groq llama-3.3-70b) → Analyst (Gemini 2.5 Flash Lite)"
            f" + Aggregator (Gemini 2.5 Flash Lite)\n\n---\n"
        )
...
def _import_project():
    """
    Import from multi_agent instead of agent.
    Every other dependency (session_store, auth, db) is identical to test_run.py.
    """
    try:
        ...
        _err("Activate your venv and ensure .env has all API keys "
             "(GEMINI_API_KEY_ANALYSIS, GEMINI_API_KEY_AGGREGATOR, GROQ_API_KEY).")
        sys.exit(1)
...
def cmd_files(session_id: str, get_files_fn):
    ...
    for i, f in enumerate(files, 1):
        exists = (_c(LIME, "✔ on disk") if os.path.exists(f["filepath"])
                  else _c(ERR_FG, "✖ missing"))
        lines.append(
            _c(MUTED,  f"  {i:<3}")
...
def cmd_history(session_id: str, get_display_history_fn):
    ...
    for m in messages:
        ...
        role = m["role"]
        body = m["content"]
        badge = _c(BG_USER, "  You  ") if role == "user" else _c(BG_ARTHA, "  Artha  ")
        _blank()
    ...
    try:
        while True:
            ...
            # ── Agent turn ────────────────────────────────────────────────────
                ...
                # Enrich message with session_id note — this is what lets the
                # Guide Agent extract and forward session_id to both specialists.
                if files:
